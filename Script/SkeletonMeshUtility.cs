    ...
    public static class SkeletonMeshUtility
    {
        class MeshBuilder
        {
            ...
            public Mesh CreateMesh()
            {
                ...
                mesh.RecalculateNormals();//重新計算計算法線
                mesh.RecalculateBounds();//從頂點重新計算網格物體的邊界體積。
                return mesh;
            }
        }
        ...
        static BoneHeadTail[] Bones = new BoneHeadTail[]
        {
            ...
            new BoneHeadTail(HumanBodyBones.Spine, HumanBodyBones.Chest, 0.1f, 0.06f),
        ...
        public static SkinnedMeshRenderer CreateRenderer(Animator animator)
        {
            ...
            {
                var head = animator.GetBoneTransform(headTail.Head);//取得骨頭位置
                if (head!=null)
                {
                    Transform tail = null;
                    ...
                    else if(headTail.Tail == HumanBodyBones.Chest) 
                    {
                        // Lack of spine
                        tail = animator.GetBoneTransform(HumanBodyBones.Neck);
                        builder.AddBone(head.position, tail.position, bones.IndexOf(head), headTail.XWidth, headTail.ZWidth);
                    }
                    else
                    {
                ...
                //else if (headTail.Head == HumanBodyBones.Spine) 
                //{
                //    head = animator.GetBoneTransform(HumanBodyBones.Spine);
                //}
                else
                {
